...
import { sendNotification } from "../../../CitizenDashboard/Components/Services/Complaint/SendNotifcation";
...
const ComplaintForm = (props) => {
	...
	const complainantname = `${loginData.validadmin?.firstName} ${loginData.validadmin?.lastName}`;
	const complainantid = `${loginData.validadmin._id}`;
	const timeAndDate = `${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}`;

	const classes = useStyles();

	const { onClose, fetchData, getFiledComplaint, getPendingComplaints, getReviewedComplaints, getUnderInvestigation } = props;
